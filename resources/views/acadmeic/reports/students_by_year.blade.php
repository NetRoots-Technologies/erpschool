        ...
        <div class="col-12">
            <div class="top-filter">
                <div class="filter-controls">
                    ...
                    <small class="text-muted d-none d-md-inline" style="margin-left:.5rem;">Select year → click Load</small>
                </div>
            ...
            <div class="card shadow-sm">
                <div class="card-header">
                    <div>
                        ...
                        <div class="card-subtitle">Detailed student list for the selected year</div>
                    </div>
                    <div class="card-actions">
                        {{-- DataTable buttons will appear above table --}}
                    </div>
                </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-sm table-bordered mb-0" id="summary_table">
                            ...
                            <tbody id="summary_body">
                                <tr>
                                    <td colspan="4" class="text-center text-muted p-4">No data — select year and click Load</td>
                                </tr>
                            </tbody>
...
$(function(){
    ...
    var table = $('#students_table').DataTable({
        ...
        initComplete: function(){
            // try to move buttons into a nicer spot
            try {
    ...
    function exportSummaryCSV(filename){
        if(!summaryCache || summaryCache.length === 0){
            alert('No summary data to export. Load a year first.');
            return;
        }
    ...
    $('#btn_clear').on('click', function(){
        ...
        table.ajax.reload();
        $('#summary_body').html(`<tr><td colspan="4" class="text-center text-muted p-3">No data — select year and click Load</td></tr>`);
        $('#summary-range').text('');
        $('#summary_totals_container').html('');
        summaryCache = [];
        $('#export_summary').attr('disabled', true);
    ...
    // Optionally auto-load current year on open:
