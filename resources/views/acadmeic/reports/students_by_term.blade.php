...
/* change badges */
.badge-increase { background:#e6f8ed; color:#0b7a3e; font-weight:600; padding:.28rem .5rem; border-radius:.375rem; }
.badge-decrease { background:#fff1f0; color:#b02a37; font-weight:600; padding:.28rem .5rem; border-radius:.375rem; }
.badge-zero    { background:#f3f4f6; color:#6b7280; padding:.28rem .5rem; border-radius:.375rem; }
...
/* small utilities */
.text-muted-sm { font-size:.85rem; color:#6b7280; }
...
<div class="container-fluid">
        ...
        <div class="col-12">
            <div class="top-filter">
                <div class="filter-controls">
                    ...
                    <select id="filter_term" class="form-select form-select-sm" style="width:160px;">
                        <option value="1">Term 1 (Jan — Jun)</option>
                        <option value="2">Term 2 (Jul — Dec)</option>
                    </select>

                    <select id="filter_year" class="form-select form-select-sm" style="width:140px;">
                        @for($y = now()->year; $y >= now()->year - 10; $y--)
                            <option value="{{ $y }}" {{ $y == now()->year ? 'selected' : '' }}>{{ $y }}</option>
                        @endfor
                    </select>

                    <small class="text-muted-sm d-none d-md-inline" style="margin-left:.5rem;">Choose term & year then click Load</small>
                </div>
    ...
    {{-- Two-column: Students table (left full width) & Summary on right (small) --}}
    <div class="row mt-3">
        <div class="col-12">
            <div class="card shadow-sm">
                <div class="card-header">
                    <div>
                        ...
                        <div class="card-subtitle">Detailed student list for the chosen term</div>
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-sm table-bordered mb-0" id="summary_table">
                            <thead>
                                <tr>
                                    <th>Class</th>
                                    <th style="width:120px" class="text-center">Opening</th>
                                    <th style="width:120px" class="text-center">Closing</th>
                                    <th style="width:140px" class="text-center">In Students</th>
                                </tr>
                            </thead>
                            <tbody id="summary_body">
                                <tr><td colspan="4" class="text-center text-muted p-3">No data — select term & year and click Load</td></tr>
                            </tbody>
...
$(function(){
    ...
    var table = $('#students_table').DataTable({
        ...
        language: { processing: '<div style="padding:6px">Loading students…</div>' },
        initComplete: function(){
            try {
                var btnContainer = $('#students_table_wrapper .col-md-6:eq(0)');
                if (btnContainer.length) table.buttons().container().appendTo(btnContainer);
    ...
    function exportSummaryCSV(filename){
        if(!summaryCache || summaryCache.length === 0){
            alert('No summary data to export. Load a term first.');
            return;
        }
        ...
        a.href = url;
        a.download = filename || 'students_summary_term.csv';
    ...
    function loadSummary(startStr, endStr){
        $('#summary-range').text(`${startStr} → ${endStr}`);
        ...
        $.getJSON("{{ route('academic.reports.students.counts_summary') }}", {
            ...
            if(summaryCache.length > 0) $('#export_summary').attr('disabled', false);

        }).fail(function(){
            $('#summary_body').html('<tr><td colspan="4" class="text-center text-danger p-3">Failed to load</td></tr>');
        });
    }
    ...
    $('#btn_clear').on('click', function(){
        ...
        table.ajax.reload();
        $('#summary_body').html('<tr><td colspan="4" class="text-center text-muted p-3">No data — select term & year and click Load</td></tr>');
        $('#summary-range').text('');
        $('#summary_totals_container').html('');
        summaryCache = [];
        $('#export_summary').attr('disabled', true);
    ...
    // Export handler
    $('#export_summary').on('click', function(){ exportSummaryCSV('students_summary_term.csv'); });

    // Optionally auto-load current term on open:
