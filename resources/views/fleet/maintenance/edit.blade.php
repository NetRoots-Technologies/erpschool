        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Edit Maintenance Record</h3>
                    <div class="card-tools">
                        <a href="{{ route('fleet.maintenance.index') }}" class="btn btn-secondary btn-sm">
                            <i class="fa fa-arrow-left"></i> Back to Maintenance
                        </a>
                        <a href="{{ route('fleet.maintenance.show', $maintenance->id) }}" class="btn btn-info btn-sm">
                            <i class="fa fa-eye"></i> View
                        </a>
                    </div>
                </div>
                <div class="card-body">
                    <form action="{{ route('fleet.maintenance.update', $maintenance->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="row">
                            <div class="col-md-6">
                                    ...
                                    <select class="form-control select2 @error('driver_id') is-invalid @enderror" 
                                        ...
                                        @foreach(\App\Models\Fleet\Driver::where('status', 'active')->get() as $driver)
                                            <option value="{{ $driver->id }}" 
                                                    {{ old('driver_id', $maintenance->driver_id) == $driver->id ? 'selected' : '' }}>
                                                {{ $driver->driver_name }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2 @error('maintenance_type') is-invalid @enderror" 
                                            id="maintenance_type" name="maintenance_type" required>
                                        <option value="">Select Type</option>
                                        <option value="regular" {{ old('maintenance_type', $maintenance->maintenance_type) == 'regular' ? 'selected' : '' }}>Regular</option>
                                        <option value="scheduled" {{ old('maintenance_type', $maintenance->maintenance_type) == 'scheduled' ? 'selected' : '' }}>Scheduled</option>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="date" class="form-control @error('maintenance_date') is-invalid @enderror" 
                                           ...
                                           value="{{ old('maintenance_date', $maintenance->maintenance_date ? $maintenance->maintenance_date->format('Y-m-d') : '') }}" required>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control @error('cost') is-invalid @enderror" 
                                           id="cost" name="cost" 
                                           value="{{ old('cost', $maintenance->cost) }}" min="0" step="0.01" required>
                                    @error('cost')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('service_provider_phone') is-invalid @enderror" 
                                           ...
                                           value="{{ old('service_provider_phone', $maintenance->service_provider_phone) }}">
                        ...
                        <div class="form-group">
                            <button type="submit" class="btn btn-primary">
                                <i class="fa fa-save"></i> Update Maintenance Record
                            </button>
