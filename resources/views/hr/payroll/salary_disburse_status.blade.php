...
<div class="container-fluid">
    ...
    @if($payrollApproval->approved == 1 && (empty($payrollApproval->disbursed) || $payrollApproval->disbursed != 1))
    <a href="{{ route('hr.salary.disburse.process',$payrollApproval->id) }}" type="submit"
        class="btn btn-success" onclick="return confirm('Are you sure you want to disburse this salary? This will create ledger entries.');">Disburse</a>
    @elseif(!empty($payrollApproval->disbursed) && $payrollApproval->disbursed == 1)
    <span class="btn btn-info" disabled>Already Disbursed</span>
    @else
    <span class="btn btn-warning" disabled>Payroll must be approved first</span>
    @endif
