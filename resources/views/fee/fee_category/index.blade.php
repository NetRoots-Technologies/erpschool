...
@section('content')
    <div class="container-fluid">
        <div class="row w-100  mt-4 ">
            <h3 class="text-22 text-center text-bold w-100 mb-4"> Fee Category</h3>
        </div>
                    ...
                    <div class="card-body table-responsive">
                        <table class="w-100 table border-top-0 table-bordered   border-bottom " id="data_table">
                            ...
                            <tr>
                                ...
                                <th class="heading_style">FA Percent</th>
                                <th class="heading_style">Full Fee</th>
                                <th class="heading_style">FA</th>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            var dataTable = $('#data_table').DataTable({
                                    ...
                                    if (selectedIds.length > 0) {
                                        ...
                                        Swal.fire({
                                            ...
                                            if (result.isConfirmed) {
                                                $.ajax({
                                                    url: '{{ route('academic.fee-category-bulk') }}',
                                                    type: 'POST',
                                                    data: {
                                                        ids: selectedIds,
                                                        "_token": "{{ csrf_token() }}",
                ...
                ajax: {
                    "url": "{{ route('datatable.getFeeCategory') }}",
                    "type": "POST",
                ...
                "columns": [
                    ...
                    { data: 'fa_percent', name: 'fa_percent' },
                    { data: 'full_fee', name: 'full_fee' },
                    { data: 'FA', name: 'FA' },
            ...
            $('#data_table tbody').on('click', '.change-status', function () {
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{route('admin.fee-category.change-status')}}',
                    data: {
                        id: id,
                        status: status,
                        _token: '{{ csrf_token() }}',
