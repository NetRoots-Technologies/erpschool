        ...
        <div class="col-md-8">
            <div class="card shadow-sm border-0">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.fee-management.collections.store-challan-payment') }}" method="POST" id="challanPaymentForm">
                        @csrf
                        ...
                        <div id="paymentDetails" style="display: none;">
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label for="collection_date" class="form-label">Payment Date <span class="text-danger">*</span></label>
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label for="paid_amount" class="form-label">Amount to Pay <span class="text-danger">*</span></label>
                                        <input type="number" class="form-control @error('paid_amount') is-invalid @enderror" 
                                               id="paid_amount" name="paid_amount" 
                                               placeholder="Enter amount" min="0" step="0.01" max="0" required>
                                        @error('paid_amount')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="form-group">
                                    <div class="row">
                                        <div class="col-md-6">
                                            <button type="submit" class="btn btn-success btn-block" id="submitPayment" disabled>
...
<script>
    $(document).ready(function() {
        ...
        $('#academic_class_id').change(function() {
            ...
            challanSelect.html('<option value="">Select student first to load challans</option>').prop('disabled', true);
            $('#challanDetails').hide();
            $('#paymentDetails').hide();
        ...
        $('#student_id').change(function() {
            ...
            if (sessionId && sessionName) {
                ...
                if (studentId) {
                    challanSelect.html('<option value="">Loading challans...</option>').prop('disabled', true);
                    
                    $.ajax({
                        url: '{{ route("admin.fee-management.collections.challans-by-student", ":studentId") }}'.replace(':studentId', studentId),
                        type: 'GET',
                        success: function(response) {
                            ...
                            if (response.challans.length > 0) {
                                response.challans.forEach(function(challan) {
                                    ...
                                    if (challan.status === 'paid') {
                                        statusText = 'Paid';
                                    } else if (challan.status === 'partial' || challan.status === 'partially_paid') {
                                        statusText = 'Partial';
                                    } else {
                                        statusText = 'Pending'; // For generated, empty, or any other status
                                    }
                                    
                                    challanSelect.append(
                                        '<option value="' + challan.id + '" ' +
                                        'data-challan-data=\'' + JSON.stringify(challan) + '\'>' +
                                        challan.challan_number + ' - ' + challan.billing_month + ' (' + statusText + ')' +
                                        '</option>'
                                    );
                                });
                                challanSelect.prop('disabled', false);
                            } else {
                                challanSelect.html('<option value="">No challans found for this student</option>');
                            }
                        },
                        error: function() {
                            challanSelect.html('<option value="">Error loading challans</option>');
                        }
                    });
                }
            } else {
        ...
        function displayChallanDetails(challan) {
            ...
            $('#billingMonth').text(new Date(challan.billing_month + '-01').toLocaleDateString('en-US', { year: 'numeric', month: 'long' }));
        ...
        function loadChallanDiscounts(challanId) {
            $.ajax({
                url: '{{ route("admin.fee-management.collections.challan-discounts", ":challanId") }}'.replace(':challanId', challanId),
                type: 'GET',
                success: function(response) {
                    if (response.discounts && response.discounts.length > 0) {
                        displayDiscounts(response.discounts, response.totalDiscount, response.finalAmount);
                    } else {
                        hideDiscountSection();
                    }
                },
        ...
        function getStatusClass(status) {
            switch(status.toLowerCase()) {
                ...
                default: return 'info'; // Default to pending for any other status
            }
        }
        ...
        $('#paid_amount').on('input', function() {
            ...
            if (paidAmount > maxPayableAmount) {
                ...
                $(this).after('<div class="invalid-feedback">Payment amount cannot exceed maximum payable amount (Rs. ' + maxPayableAmount.toLocaleString() + ')</div>');
            } else {
        ...
        $('#challanPaymentForm').on('submit', function(e) {
            ...
            if (paidAmount > maxPayableAmount) {
                e.preventDefault();
                toastr.error('Payment amount cannot exceed maximum payable amount (Rs. ' + maxPayableAmount.toLocaleString() + ')');
                return false;
            }
