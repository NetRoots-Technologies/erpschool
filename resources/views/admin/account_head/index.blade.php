...
<div class="container-fluid">
    <div class="row w-100  mt-4 ">
        <h3 class="text-22 text-center text-bold w-100 mb-4"> Account Head </h3>
    </div>
    <div class="row    mt-4 mb-4 ">
    @if (Gate::allows('AccountHead-create'))
        <div class="col-12 text-right">
            <a class="btn btn-primary btn-md text-white" data-toggle="modal" data-target="#createModal1"><b>Add Account
                    Head</b></a>
        </div>
        @endif
    </div>
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-header">
                                <h4 class="modal-title">Create Account Head</h4>
        ...
        <script type="text/javascript">
            ...
            var tableData = null;
                        tableData = $('#file-datatable').DataTable({
                                                ...
                                                if (selectedIds.length > 0) {
                                                    ...
                                                    Swal.fire({
                                                        ...
                                                        if (result.isConfirmed) {
                                                            $.ajax({
                                                                url: '{{ route('admin.account-head-bulk') }}',
                                                                type: 'POST',
                                                                data: {
                                                                    ids: selectedIds,
                                                                    "_token": "{{ csrf_token() }}",
                            ...
                            ajax: {
                                "url": "{{ route('datatable.account-head.getdata') }}",
                                "type": "POST",
                    ...
                    $('#create-form-submit').on('click', function (e) {
                        ...
                        var url = "{{ route('admin.account-head.store') }}";
                    ...
                    $('#file-datatable tbody').on('click', '.account_head', function () {

                        var id = $(this).data('account-edit').id;
                        var name = $(this).data('account-edit').name;
                    ...
                    $('#tag-form-submit').on('click', function (e) {
                        e.preventDefault();
                        var id = $('#edit_id').val();
                        var url = "{{ route('admin.account-head.index') }}";
                        var loader = $('<div class="loader"></div>').appendTo('body');
                        ...
                        $.ajax({
                            ...
                            error: function () {
                                ...
                                toastr.error('Error while updating account head.');
                            }
                        });
                        return false;
                    });
                    ...
                    $('#file-datatable tbody').on('click', '.change-status', function () {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: '{{route('admin.account-head.change-status')}}',
                            data: {
                                id: id,
                                status: status,
                                _token: '{{ csrf_token() }}',
