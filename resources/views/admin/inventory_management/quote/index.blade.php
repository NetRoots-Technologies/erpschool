...
@section('js')
    ...
    window.location.href = `/inventory/quotes/${itemType}/${id}`;
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row, meta) {
                            let nData = row.items.map(x =>
                                `<span class="badge bg-success rounded-pill m-1">${x.name}</span>`
            ...
            function renderItems(dt, isEdit = false) {
                let html = ``;
                if (dt && Array.isArray(dt.quote_items) && dt.quote_items.length > 0) {
                    const quoteItems = dt?.quote_items || [];
                ...
                <div class="row m-3">
                    ...
                    <input type="hidden" name="item_id[${i}]" value="${q.item_id}">
                    <input type="text" class="form-control"
                            value="${item.name ?? ''} (${item.measuring_unit ?? q.measuring_unit ?? ''})"
                            disabled required>
                    </span>

                    <span class="col-2 mb-3">
                    <label class="form-label">Quantity</label>
                    <input type="number" min="1" step="1" class="form-control quantity"
                            name="quantity[${i}]"
                            value="${q.quantity ?? 0}" required>
                    </span>

                    <span class="col-2 mb-3">
                    <label class="form-label">Price</label>
                    <input type="number" min="1" class="form-control item_price"
                            name="price[${i}]"
                            value="${q.unit_price ?? 0}" required>
                    </span>
                    ...
                    <input type="number" class="form-control total"
                            name="total[${i}]"
                            value="${q.total_price ?? 0}" readonly>
                    </span>
