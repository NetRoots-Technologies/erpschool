    ...
    $(document).ready(function () {
        ...
        let dt = $('#data_table').DataTable({
            ...
            columns: [
                ...
                {
                    data: 'purchaseOrderItems', title: 'Items', width: "20%",
                    render: function (data, type, row, meta) {
                        let nData = row.purchase_order_items.filter(x => x.item != null).map(x => `<span class="badge bg-success rounded-pill m-1">${x.item.name} ${x.quantity}</span>`).sort((a, b) => b.length - a.length).join('&nbsp')
                        return nData
                    }
                },
                {
