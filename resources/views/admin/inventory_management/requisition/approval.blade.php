                ...
                <form id="iForm" method="get">
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="comments" name="comments" placeholder="Write Reason" required></textarea>
                        </div>
                    </div>
                ...
                columns: [{
...

                    {
                        ...
                        render: function(data, type, row, meta) {
                            ...
                            if (
                                row.item &&
                                row.item.inventory &&
                                row.item.inventory.item_id &&
                                row.item.id === row.item.inventory.item_id
                            ) {
                                return row.item.inventory.quantity ?? "0";
                            } else {
                                return 0;
                            }
                        }
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            const hasInventory = row.item && row.item.inventory;
                            const insufficientStock = hasInventory && row.item.inventory.quantity <
                                row.quantity;
                            const noInventory = !hasInventory;
                            const disableApprove = noInventory || insufficientStock;

                            if (row.status === 'APPROVED') {
                                return `<span class="badge bg-success rounded-pill">${row.status}</span>`;
                            }
                            if (row.status === 'REJECTED') {
                                return `<span class="badge bg-danger rounded-pill">${row.status}</span>`;
                            }

                            const approveBtn = disableApprove ?
                                `<span class="btn btn-sm btn-success accept disabled" title="${noInventory ? 'No inventory record' : 'Insufficient stock'}">Approve</span>` :
                                `<span class="btn btn-sm btn-success accept" data-uri="${changeStatusUri}/${row.id}">Approve</span>`;

                            const rejectBtn =
                                `<span class="btn btn-sm btn-danger reject" data-uri="${changeStatusUri}/${row.id}">Reject</span>`;
