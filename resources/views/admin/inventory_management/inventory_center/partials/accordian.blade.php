    ...
    <div class="card mb-0 overflow-visible">
            ...
            <div class="dropdown position-absolute" style="top: 10px; right: 15px;">
                ...
                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenu{{ $category->id }}">
                    ...
                    <button class="dropdown-item create-btn" data-id="{{$category->id}}">
                        <i class="fas fa-create mr-1"></i> Create
                    </button>
                    @endif
                    @if ($category->level > 1)
                        ...
                        <form action="{{ route('inventory.inventory-center.destroy', $category->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
        ...
        <div id="{{ $collapseId }}" class="collapse border border-card p-3" aria-labelledby="{{ $headingId }}"
            ...
            <div class="card-body bg-white">
               ...
               {{-- Show inventorys only if this is a leaf node --}}
                @if ($category->recursiveChildren->isEmpty() && $category->inventorys->isNotEmpty())
                        <ul class="list-group">
                            @foreach ($category->inventorys as $inventory)
                                <li>
                                    <div>
                                        <strong>{{ $inventory->code. ' - ' . $inventory->name }}</strong>
                                    </div>
                                </li>
                            @endforeach
                        </ul>
                ...
                @foreach ($category->recursiveChildren as $child)
                    @include('admin.inventory_management.inventory_center.partials.accordian', ['category' => $child])
                @endforeach
            </div>
        </div>
    </div>
