...
<div class="container-fluid">
    <div class="row w-100  mt-4 ">
        <h3 class="text-22 text-center text-bold w-100 mb-4"> Exam Detail</h3>
    </div>
    <div class="row    mt-4 mb-4 ">
        @if (Gate::allows('ExamDetails-create'))
            <div class="col-12 text-right">
                <a href="{!! route('exam.exam_details.create') !!}" class="btn btn-primary btn-md"><b>Add Exam
                        Detail</b></a>
            </div>
        @endif
    </div>
                    ...
                    <table class="w-100 table border-top-0 table-bordered   border-bottom " id="data_table">
                        <thead>
                            <tr>
                                ...
                                <th class="heading_style">Test Name</th>
                                <th class="heading_style">Initial</th>
                                <th class="heading_style">Created By</th>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            var dataTable = $('#data_table').DataTable({
                                    ...
                                    if (selectedIds.length > 0) {
                                        ...
                                        Swal.fire({
                                            ...
                                            if (result.isConfirmed) {
                                                $.ajax({
                                                    url: '{{ route('exam.exam-details-bulk') }}',
                                                    type: 'POST',
                                                    data: {
                                                        ids: selectedIds,
                                                        "_token": "{{ csrf_token() }}",
                ...
                ajax: {
                    "url": "{{ route('datatable.getExamDetail') }}",
                    "type": "POST",
            ...
            $(document).on('click', '.delete-btn', function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '/exam/exam_details/' + id, // <-- adjust if route prefix differs
                            type: 'POST',
                            data: {
                                _method: 'DELETE',
                                _token: "{{ csrf_token() }}",
