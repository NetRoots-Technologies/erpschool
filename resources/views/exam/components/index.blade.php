...
<div class="container-fluid">
    <div class="row w-100  mt-4 ">
        <h3 class="text-22 text-center text-bold w-100 mb-4"> Components</h3>
    </div>
    <div class="row    mt-4 mb-4 ">
        @if (Gate::allows('Components-create'))
            <div class="col-12 text-right">
                <a href="{!! route('exam.components.create') !!}" class="btn btn-primary btn-md"><b>Add Components</b></a>
            </div>
        @endif
    </div>
...
<div class="modal fade" id="cloneModal" tabindex="-1" aria-labelledby="cloneModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form id="cloneForm" method="POST" action="{{ route('exam.component.clone') }}">
                @csrf
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            var dataTable = $('#data_table').DataTable({
                                    ...
                                    if (selectedIds.length > 0) {
                                        ...
                                        Swal.fire({
                                            ...
                                            if (result.isConfirmed) {
                                                $.ajax({
                                                    url: '{{ route('exam.components-bulk') }}',
                                                    type: 'POST',
                                                    data: {
                                                        ids: selectedIds,
                                                        "_token": "{{ csrf_token() }}",
                ...
                ajax: {
                    "url": "{{ route('datatable.component.getdata') }}",
                    "type": "POST",
            ...
            $('tbody').on('click', '.change-status', function () {
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{route("exam.components.change-status")}}',
                    data: {
                        id: id,
                        status: status,
                        _token: '{{ csrf_token() }}',
