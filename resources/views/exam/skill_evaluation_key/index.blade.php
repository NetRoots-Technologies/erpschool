...
@section('content')
    ...
    <div class="container-fluid">
        <div class="row w-100  mt-4 ">
            <h3 class="text-22 text-center text-bold w-100 mb-4">Skill Evaluation Key</h3>
        </div>
        <div class="row    mt-4 mb-4 ">
       @if (Gate::allows('SkillEvaluationsKey-create'))
            <div class="col-12 text-right">
                <a class="btn btn-primary btn-md text-white" data-toggle="modal" data-target="#createModal1"><b>Add Evaluation Key</b></a>
            </div>
            @endif
        </div>
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-header">
                                    <h4 class="modal-title">Create Evaluation Key</h4>
                ...
                <script type="text/javascript">
                    ...
                    $(document).ready(function () {
                        ...
                        tableData = $('#file-datatable').DataTable({
                                                ...
                                                if (selectedIds.length > 0) {
                                                    ...
                                                    Swal.fire({
                                                        ...
                                                        if (result.isConfirmed) {
                                                            $.ajax({
                                                                url: '{{ route('exam.skill-evaluation-key-bulk') }}',
                                                                type: 'POST',
                                                                data: {
                                                                    ids: selectedIds,
                                                                    "_token": "{{ csrf_token() }}",
                            ...
                            ajax: {
                                "url": "{{ route('datatable.skill-evaluation-key.getdata') }}",
                                "type": "POST",
                    ...
                    $('#file-datatable tbody').on('click', '.skillEvaluationKey_edit', function () {
                        var id = $(this).data('skill_evaluation-edit').id;
                        var abbr = $(this).data('skill_evaluation-edit').abbr;
                        var key = $(this).data('skill_evaluation-edit').key;
                    ...
                    $('#file-datatable tbody').on('click', '.change-status', function () {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: '{{route('exam.skillEvaluationKey.change-status')}}',
                            data: {
                                id: id,
                                status: status,
                                _token: '{{ csrf_token() }}',
                            ...
                            error: function (xhr, status, error) {
                                loader.remove();
                                toastr.success('Error while updating Status')
                                console.error(xhr.responseText);
                            }
                        });
                    });
