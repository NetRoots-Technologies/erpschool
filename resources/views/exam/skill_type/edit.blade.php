        ...
        <div class="row">
            <div class="col-12">
                <div class="card basic-form">
                    <div class="card-body">
                        <h3 class="text-22 text-midnight text-bold mb-4"> Update skill Type</h3>
                        <div class="row mt-4 mb-4 ">
                            <div class="col-12 text-right">
                                <a href="{!! route('exam.skill_types.index') !!}"
                        ...
                        @endif
                        <form action="{!! route('exam.skill_types.update',$skillType->id) !!}" enctype="multipart/form-data"
                                        ...
                                        <div class="row mt-3">
                                            ...
                                            <div class="col-md-4">
                                                ...
                                                <select name="company_id"
                                                    ...
                                                    @foreach($companies as $item)
                                                        <option value="{{$item->id}}" {!! $skillType->company_id == $item->id ? 'selected' : '' !!}>{{$item->name}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            ...
                                            <div class="col-md-4">
                                                ...
                                                <select name="session_id"
                                                        ...
                                                        aria-label=".form-select-lg example" required>
                                                    <option value="" selected required>Select Session</option>
                                                    @foreach($sessions as $key => $item)
                                                        <option value="{{$key}}" {!! $skillType->session_id == $key ? 'selected' : '' !!}>{{$item}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                <select name="group_id"
                                                    ...
                                                    @foreach($groups as  $item)
                                                        <option value="{{$item->id}}" {!! $skillType->group_id == $item->id ? 'selected' : '' !!}>{{$item->skill_group}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                <select name="skill_id" class="form-select select2 basic-single mt-3" aria-label=".form-select-lg example" required>
                                                    ...
                                                    @foreach($skills as $skill)
                                                        <option value="{{ $skill->id }}" {!! $skillType->skill_id == $skill->id ? 'selected' : '' !!}>{{$skill->name}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        data.forEach(function (branch) {
                            var selectBranch = branch.id == '{{ $skillType->branch_id }}' ? 'selected' : '';
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        data.forEach(function (academic_class) {
                            var selectsection = academic_class.id == '{{ $skillType->class_id }}' ? 'selected' : '';
    ...
    <script>
        ...
        $(document).ready(function () {
            $('.select_class').on('change', function () {
                class_id = $(this).val();
                if(class_id == null){
                    class_id = {!! $skillType->class_id !!}
                }

                $.ajax({
                    type: 'GET',
                    url: '{{ route('exam.fetchSubjects') }}',
                    data: {
                        class_id: class_id
                    },
                    success: function (data) {
                        ...
                        data.forEach(function (academic_class) {
                            ...
                            var selectsubject = academic_class.id == '{{ $skillType->subject_id }}' ? 'selected' : '';
                            courseDropdown.append('<option value="' + academic_class.id + '" ' + selectsubject + '>' + academic_class.name + '</option>');
                        });
