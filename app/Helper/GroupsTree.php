...
class GroupsTree
{
    ...
    function add_sub_groups()
    {
        ...
        if ($this->id == 0) {
            $child_group_q = Group::where(['parent_id' => $this->id])->OrderBy('code', 'asc')->get()->toArray();
        } else {
            $child_group_q = Group::where(['parent_id' => $this->id])->OrderBy('code', 'asc')->get()->toArray();
        }
    ...
    public static function buildOptions(array $arr, $target, $parent = NULL)
    {
        ...
        foreach ($arr as $key => $v) {
            if ($v['id'] == $target)
                ...
                $html .= "<option value='" . $v['id'] . "' selected='selected'>$parent {$v['code']} - {$v['name']}</option>";
            else
                //                $html .= "<option value='" . $v['id'] . "'>$parent {$v['name']}</option>\n";
                $html .= "<option value='" . $v['id'] . "'>$parent {$v['code']} - {$v['name']}</option>";
   ...
   //Just for disable above 3 level groups
   
   public static function buildOptions2(array $arr, $target, $parent = NULL)
    {
        $html = "";

        foreach ($arr as $key => $v) {
            $selected = ($v['id'] == $target) ? "selected='selected'" : "";
            $disabled = ($v['level'] > 3) ? "disabled" : "";

            $html .= "<option value='{$v['id']}' {$selected} {$disabled}>$parent {$v['code']} - {$v['name']}</option>";

            if (array_key_exists('children', $v)) {
                $html .= self::buildOptions2($v['children'], $target, $parent . str_repeat('&nbsp;', 10));
            }
        }
   ...
   public static function buildOptions3(array $arr, $target, $parent = NULL)
    {
        $html = "";
        ...
        foreach ($arr as $key => $v) {
            ...
            $html .= "<option value='{$v['id']}' {$selected} {$disabled}>$parent {$v['number']} - {$v['name']}</option>";
