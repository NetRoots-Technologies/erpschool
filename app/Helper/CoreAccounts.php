...
class CoreAccounts
{
    ...
    static function generateNumberLetter($alpha, $number)
    {
        return "$alpha#" . sprintf('%07d', $number);
    }
    ...
    static function generateLedgerNumber($companyId, $groupNumber)
    {
        ...
        return $groupNumber . '-' . sprintf('%04d', (Ledgers::where(['group_id' => $companyId])->count()));
    }
    ...
    private static function generateLevelAndNumber($parent_id, $adittional_number = 0)
    {
        ...
        $currentLevel = $ParentGroup->level;

        // Cap the level at 4
        if ($currentLevel >= 4) {
            throw new \Exception('Maximum group level (4) reached. Cannot create sub-group.');
        }

        // Generate new level and number
        $newLevel = $currentLevel + 1;
        $segmentCount = count(explode('-', $ParentGroup->number)) + 1;
        // $newNumber = $ParentGroup->number . '-' . sprintf('%0' . $segmentCount . 'd', Group::where(['parent_id' => $ParentGroup->id])->count() + $adittional_number + 1);
        $lastItem = Group::where(['parent_id' => $ParentGroup->id])->orderBy('id', 'desc')->value('number');
        // dd($lastItem,substr($lastItem,strlen($lastItem)-1,1)+1 );
        $newNumber = $ParentGroup->number . '-' . sprintf('%0' . $segmentCount . 'd', (int) substr($lastItem, strlen($lastItem) - 1, 1) + 1);
        // dd($newNumber);
        return [
            'number' => $newNumber,
            'level' => $newLevel,
    ...
    public static function createGroup($data)
    {
        ...
        if ($validator->fails()) {
            return [
                'status' => false,
                'error' => $validator->errors()->first() // return first validation error
            ];
        }

        if (!$Group = Group::find($data['parent_id'])) {
            return [
                'status' => false,
                'error' => 'Parent Group ID does not exist.'
            ];
        }
    ...
    public static function createGoldPurchaseEntry($data)
    {
        ...
        $Stock_ledger = Group::where(['parent_id' => 18])->where(['parent_type' => $data['item_cate']])->first();
    ...
    {
        $Tagg_itemsID = Group::where(['parent_id' => Config::get('constants.acounts_process_tag')])->where(['parent_type' => $data['lds_type']])->value('id');
        $Tagg_itemsID = Ledgers::where('group_id', $Tagg_itemsID)->value('id');
        $stock_itemsID = Group::where(['parent_id' => Config::get('constants.acounts_sale_tag')])->where(['parent_type' => $data['lds_type']])->value('id');
    ...
    public static function createLDSPurchaseEntry($data, $stock = null, $acc_data)
    {
        ...
        foreach ($stock as $key) {
            if ($key['shift_to_tagging'] == 'Inventory') {
                $stock_itemsID = Group::where(['parent_id' => Config::get('constants.acounts_sale_pack')])->where(['parent_type' => $acc_data['lds_type']])->value('id');
                $Ineventory_items = Ledgers::where('parent_type', $key['packet_no'])->where('group_id', $stock_itemsID)->value('id');
            } else {
                //$stock_itemsID =  Group::where(['parent_id' => Config::get('constants.acounts_process_tag')])->where(['parent_type' => $data['lds_type']])->value('id');
    ...
    public static function updateGroup($data, $id)
    {
        ...
        if ((Group::hasChildLedgers($Group->id) || Group::hasChildGroups($Group->id)) && $Group->parent_id != $data['parent_id']) {
            return array(
                'status' => false,
                'error' => 'Parent Group can not be changed due to one or more Group(s) / Ledger(s) are associated with."' . $Group->name . '" group.'
            );
        } else {
            if (!$ParentGroup = Group::where(['id' => $data['parent_id']])->first()) {
                return array(
                    'status' => false,
     ...
     * @return (array) $response
     */
    public static function createLedger($data,$balance=0)
    {
        // Get selected group
        if (!$Group = Group::where(['id' => $data])->first()) {
            return array(
                'status' => false,
                'error' => 'Group ID does not exists'
            );
        } else { 
            if ($Group->account_type_id == 1 || $Group->account_type_id == 3) { //checs if its asset or liablity
                $balance_type = 'd';
            } else {
                $balance_type = 'c';
            }
            $Ldata['name'] = $Group->name;
            $Ldata['group_id'] = $Group->id;
            $Ldata['code'] = $Group->number;
            $Ldata['balance'] = $balance;
    ...
    public static function updateLedger($data, $id)
    {
        ...
        // Get selected group
        if (!$Group = Group::where(['id' => $data['group_id']])->first()) {
            return array(
                'status' => false,
                'error' => 'Group ID does not exists'
            );
...
//////////core code written by Muhammad azeem khalid
    ...
    public static function closing_balance($ledgerID, $cID = 0, $df = "", $dt = "")
    {
        ...
        $tcr = 0;
        foreach ($Currencies as $currency) {
            $ob = self::opening_balance($df, 1, $ledgerID, $currency->currency_id);
            $currency_symbol = Currencies::where('id', $currency->currency_id)->first(['code', 'decimal_fixed_point', 'rate']);
            $dr = EntryItems::where(['ledger_id' => $ledgerID, 'other_currency_type' => $currency->currency_id, 'dc' => 'd'])->whereBetween('voucher_date', [$df, $dt])->get()->sum('amount');
            $cr = EntryItems::where(['ledger_id' => $ledgerID, 'other_currency_type' => $currency->currency_id, 'dc' => 'c'])->whereBetween('voucher_date', [$df, $dt])->get()->sum('amount');
            $balance = $ob + $dr - $cr;
            if ($ob > 0) {
                $tdr = $ob + $dr;
                $opening_balance = $ob;
